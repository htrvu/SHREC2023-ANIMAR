...
from pytorch_metric_learning.losses import NTXentLoss, CrossBatchMemory, ArcFaceLoss
...
from common.train import train_loop, train_loop_arcface, val_loop, val_loop_arcface
...
parser.add_argument('--loss-func', type=str, default='ntxent', choices=['ntxent', 'arcface'], help='Loss function')
...
if args.loss_func == 'ntxent':
    print('Training with NTXent loss...')
...
elif args.loss_func == 'arcface':
    print('Training with Arcface loss...')
    loss_func = ArcFaceLoss(num_classes=711, embedding_size=args.latent_dim, scale=16)

    # Set optimizers
    optimizer1 = torch.optim.AdamW([
        {
            'params': obj_embedder.parameters(),
            'lr': args.lr_obj,
            'weight_decay': 0.0001
        },
        {
    ...
    optimizer2 = torch.optim.AdamW([
        {
            'params': query_embedder.parameters(),
            'lr': args.lr_skt,
            'weight_decay': 0.0001
        },
        {
    ...
    for e in range(epoch):
        print(f'Epoch {e+1}/{epoch}:')
        train_loss = train_loop_arcface(obj_embedder=obj_embedder, query_embedder=query_embedder,
                        obj_input='object_ims', query_input='query_ims',
                        loss_func=loss_func,
                        obj_optimizer=optimizer1, query_optimizer=optimizer2,
                        dl=train_dl,
        ...
        val_loss = val_loop_arcface(obj_embedder=obj_embedder, query_embedder=query_embedder,
                            obj_input='object_ims', query_input='query_ims',
                            loss_func=loss_func,
                            dl=test_dl,
                            device=device)
