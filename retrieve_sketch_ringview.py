...
parser.add_argument('--exps-path', type=str, required=True, help='Path to exps folder')
...
#Info json
with open(os.path.join(args.exps_path, 'args.json')) as json_file:
...
# Load Model
## Object extraction
obj_kwargs, obj_state = torch.load(os.path.join(args.exps_path, 'weights', 'best_obj_embedder.pth'))
...
## sketch extraction
query_kwargs, query_state = torch.load(os.path.join(args.exps_path, 'weights', 'best_query_embedder.pth'))
